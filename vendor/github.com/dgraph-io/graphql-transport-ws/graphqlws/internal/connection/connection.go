...
func Connect(ws wsConnection, service GraphQLService, headers http.Header, options ...func(conn *connection)) func() {
	conn := &connection{
		service: service,
		ws:      ws,
	}
	...
	conn.cancel = cancel
	conn.readLoop(ctx, headers, conn.writeLoop(ctx))
...
func (conn *connection) readLoop(ctx context.Context, reqHeaders http.Header, send sendFunc) {
	defer conn.close()
	...
	for {
		...
		case typeStart:
			...
			if reqHeaders != nil {
				opCtx = context.WithValue(opCtx, "RequestHeader", reqHeaders)
			}
